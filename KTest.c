...
enum { CLIENT_TO_SERVER=0, SERVER_TO_CLIENT, RNG, PRNG, TIME, TOD, PID, ADDRINFO, ADDR,
    STDIN, SELECT, MASTER_SECRET };
static char* ktest_object_names[] = {
  "c2s", "s2c", "rng", "prng", "time", "time_of_day", "pid", "addrinfo", "stdin",
  "select", "master_secret"
};
...
int ktest_gettimeofday(struct timeval *tv, struct timezone *tz)
{
  assert(tz == NULL);
  if (tv == NULL) return 0;
  if (ktest_mode == KTEST_NONE) {
    ...
    KTOV_append(&ktov, ktest_object_names[TOD], sizeof(struct timeval), tv);
    if( ret != 0){
        printf("ktest_gettimeofday gettimeofday failed\n");
        exit(-1);
    }
    return ret;
  } else if (ktest_mode == KTEST_PLAYBACK) {
    KTestObject *o = KTOV_next_object(&ktov,
                            ktest_object_names[TOD]);
    struct timeval *retrieved = (struct timeval*)o->bytes;
    tv->tv_sec = retrieved->tv_sec;
    tv->tv_usec = retrieved->tv_usec;
    return 0;
  } else {
...
pid_t ktest_getpid(void)
{
  if (ktest_mode == KTEST_NONE) {
    return getpid();
  } else if (ktest_mode == KTEST_RECORD) {
    pid_t ret = getpid();
    KTOV_append(&ktov, ktest_object_names[PID], sizeof(ret), &ret);
    return ret;
  } else if (ktest_mode == KTEST_PLAYBACK) {
    KTestObject *o = KTOV_next_object(&ktov,
                            ktest_object_names[PID]);
    pid_t retrieved = *( (pid_t*)o->bytes);
    return retrieved;
  } else {
    perror("ktest_getpid coding error - should never get here");
    exit(4);
  }
}
...
int ktest_getaddrinfo(const char *node, const char *service,
     ...
     printf("libtirpc ktest_getaddrinfo\n");
     if (ktest_mode == KTEST_NONE) {
        ...
        KTOV_append(&ktov, ktest_object_names[ADDRINFO], sizeof(struct addrinfo), *res);
        KTOV_append(&ktov, ktest_object_names[ADDR], sizeof(struct sockaddr), (*res)->ai_addr);
        if(ret != 0){
        ...
        KTestObject *o = KTOV_next_object(&ktov,
                            ktest_object_names[ADDRINFO]);
        *res = (struct addrinfo *)(o->bytes);
        KTestObject *o2 = KTOV_next_object(&ktov, ktest_object_names[ADDR]);
        (*res)->ai_addr = (struct sockaddr*)(o2->bytes);
        return 0;
     } else {
        printf("ktest_getaddrinfo coding error - should never get here\n");
        exit(4);
     }
}
...
int ktest_getsockname(int sockfd, struct sockaddr *addr, socklen_t *addrlen){
    *addrlen = 16;
    ((struct sockaddr_storage*)addr)->ss_family = 2;
}
